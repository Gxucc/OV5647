...
void app_main(void)
{
    ESP_LOGI(TAG, "========== Audio Recorder Test ==========");

    ESP_ERROR_CHECK(init_i2c());
    ESP_ERROR_CHECK(audio_recorder_system_init(i2c_bus));
    ...
    audio_recorder_cfg_t cfg = {
        ...
        .channel_count = 1,
        .duration_ms = 10000,
        .save_path = "/sdcard/rec_test_10s.wav",
    };

    // 开始录音（非阻塞）
    ESP_LOGI(TAG, ">>> Starting recording (non-blocking) <<<");
    ESP_ERROR_CHECK(audio_recorder_start(&cfg));

    // 主循环可以做其他事情，比如更新 UI
    int seconds = 0;
    while (audio_recorder_is_running()) {
        ESP_LOGI(TAG, "Recording... %d seconds", seconds++);
        vTaskDelay(pdMS_TO_TICKS(1000));
    }

    ESP_LOGI(TAG, ">>> Recording finished <<<");

    audio_recorder_system_deinit();
