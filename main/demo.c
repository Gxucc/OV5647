...
// static QueueHandle_t s_face_recog_queue = NULL;      // 新增
// static QueueHandle_t s_face_recog_result_queue = NULL; // 新增

// static detect_result_t s_prev_result = {0};
// static fall_classifier_result_t s_prev_fall_result = {0};
// static face_recognition_result_wrapper_t s_prev_face_result = {0};  // 新增
// static bool s_has_new_result = false;
// static bool s_has_new_fall_result = false;
// static bool s_has_new_face_result = false;  // 新增
...
// static TaskHandle_t s_face_recog_task_handle = NULL;  // 新增

// // 任务暂停控制（新增）
// static volatile bool s_face_mode_active = false;  // 人脸识别模式激活时，暂停其他AI任务
...
//     ESP_LOGI(TAG, "face_recognition_task started");

//     while (1) {
//         if (xQueueReceive(s_face_recog_queue, &frame, portMAX_DELAY) == pdTRUE) {
//             result.detected = face_recognition_run(frame.rgb_buf, CAM_WIDTH, CAM_HEIGHT, &result.result);
//             xQueueSend(s_face_recog_result_queue, &result, portMAX_DELAY);
//             cam_net_release_rgb565();
//         }
//     }
// }
...
//         if (xQueueReceive(s_face_recog_result_queue, &face_result, 0) == pdTRUE) {
...
//             if (s_prev_face_result.detected) {
...
//                 } else {
//                     ESP_LOGI(TAG, ">>> [陌生人] 最佳相似度=%.3f", s_prev_face_result.result.similarity);
//                 }
//             }
...
//             if (xQueueSend(s_face_recog_queue, &frame, 0) != pdPASS) {
//                 cam_net_release_rgb565();
//             }
//         }
//         // ==================== 跌倒检测界面显示 ====================
...
//             }
//         }
//         // ==================== 其他界面（Home/Record/Babysound）====================
//         else {
//             // 只获取帧但不显示摄像头画面（LVGL界面覆盖）
//             cam_net_release_rgb565();
//         }
...
// {
//     while (1) {
//         ui_state_t prev_state = lvgl_ui_get_state();
...
//         }
//         if (lvgl_ui_should_start_face_recognition()) {  // 新增
...
//         ui_state_t new_state = lvgl_ui_get_state();

//         // 处理人脸识别模式的任务暂停/恢复（新增）
//         if (new_state == UI_STATE_FACE_RECOGNITION && prev_state != UI_STATE_FACE_RECOGNITION) {
//             // 进入人脸识别模式
//             s_face_mode_active = true;
//             ESP_LOGI(TAG, "Face recognition mode ACTIVATED, other AI tasks suspended");
//         } else if (new_state != UI_STATE_FACE_RECOGNITION && prev_state == UI_STATE_FACE_RECOGNITION) {
//             // 退出人脸识别模式
//             s_face_mode_active = false;
//             ESP_LOGI(TAG, "Face recognition mode DEACTIVATED, other AI tasks resumed");
//         }
...
//         // 处理人脸注册按钮（新增）
//         if (lvgl_ui_should_register_face()) {
//             lvgl_ui_clear_register_face();
//             ESP_LOGI(TAG, "Register face button clicked - capturing current frame...");
//             // 注册逻辑：使用当前帧注册人脸
//             // 实际实现需要在 display_task 中配合，这里设置标志位
//             // 简化处理：直接调用注册API（需要当前帧，实际可能需要更复杂的同步）
//             // 这里仅打印日志，实际注册需要获取当前摄像头帧
//         }
...
//     ESP_LOGI(TAG, "Delayed WiFi/MQTT init starting...");
    
//     esp_err_t ret = audio_mqtt_sender_init();
//     if (ret != ESP_OK) {
//         ESP_LOGW(TAG, "WiFi/MQTT init failed: %s", esp_err_to_name(ret));
//     } else {
//         ESP_LOGI(TAG, "WiFi/MQTT init started successfully");
//     }
...
//     ESP_ERROR_CHECK(face_recognition_init(0.65f));  // 新增：人脸识别初始化
...
//     s_face_recog_queue = xQueueCreate(2, sizeof(cam_frame_t));           // 新增
//     s_face_recog_result_queue = xQueueCreate(2, sizeof(face_recognition_result_wrapper_t)); // 新增
...
//     xTaskCreatePinnedToCore(babysound_task, "babysound", 8192, NULL, 1, &s_babysound_task_handle, 0);
//     xTaskCreatePinnedToCore(face_recognition_task, "face_recog", 8192, NULL, 4, &s_face_recog_task_handle, 1); // 新增
